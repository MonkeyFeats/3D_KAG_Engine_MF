...
namespace Collidables
{
    ...
    shared class Capsule : BoundingShape
    {
        ...
        float Length { get { return HalfLength * 2; } set { HalfLength = value * 0.5f; } }
        ...
        void ComputeInertia(float mass, BodyInertia out inertia)
        {
        ...
        const int Id = 1;
        int TypeId {  get { return Id; } }
    }

    struct CapsuleWide : IShapeWide<Capsule>
    {
        Vector<float> Radius;
        Vector<float> HalfLength;

        
        void Broadcast(Capsule shape)
        {
        ...
        void WriteFirst(Capsule source)
        {
            Unsafe.As<Vector<float>, float>(ref Radius) = source.Radius;
            Unsafe.As<Vector<float>, float>(ref HalfLength) = source.HalfLength;
        }

        bool AllowOffsetMemoryAccess => true;
        int InternalAllocationSize => 0;
        void Initialize(RawBuffer memory) { }

        
        void WriteSlot(int index, in Capsule source)
        {
        ...
        void RayTest(ref RigidPoses pose, ref RayWide ray, out Vector<int> intersected, out Vector<float> t, out Vector3Wide normal)
        {
            //It's convenient to work in local space, so pull the ray into the capsule's local space.
    ...
    struct CapsuleSupportFinder : ISupportFinder<Capsule, CapsuleWide>
    {
        bool HasMargin
        {
        ...
        void GetMargin(in CapsuleWide shape, out Vector<float> margin)
        {
            margin = shape.Radius;
        }
        
        void ComputeSupport(in CapsuleWide shape, in Matrix3x3Wide orientation, in Vector3Wide direction, in Vector<int> terminatedLanes, out Vector3Wide support)
        {
        ...
        void ComputeLocalSupport(in CapsuleWide shape, in Vector3Wide direction, in Vector<int> terminatedLanes, out Vector3Wide support)
        {
